...
class LocationOut(LocationBase):
    ...
    # Derived from `boundary_geojson` (`Location.area_ha`), never stored —
    # `None` when there's no drawn boundary.
...
class DeforestationCheckOut(BaseModel):
    """Result of checking the talhão's drawn boundary against INPE's own
...
class WeeklyReportOut(BaseModel):
    ...
    location_name: str
    crop: str | None
    # Derived from the talhão's drawn boundary — `None` without one.
    area_ha: float | None
    period_start: date
    period_end: date
    ...
    # None when the check is disabled or never ran for this talhão yet —
    # distinct from an empty `checked_sources` list, which means it ran but
    # every source failed that time (see DeforestationCheckOut).
    deforestation: DeforestationCheckOut | None = None
    # None when the source is disabled or unavailable this time — never a
    # second source of the rainfall numbers, only added context.
    soil_moisture: SoilMoistureOut | None = None
    generated_at: datetime
    # Short natural-language reading of the numbers above, generated by
    # Claude (see app/locations/ai_summary.py) — `None` when
    # ANTHROPIC_API_KEY isn't configured or the call itself fails; never a
